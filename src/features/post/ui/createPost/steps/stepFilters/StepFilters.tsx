...
import { IMAGE_FILTERS, IMAGE_FILTERS_LIST, FilterKey } from '@/features/post/types/postTypes'
...
export const StepFilters = () => {
  ...
  const handleFilterSelect = (filter: FilterKey) => setImageEditData({ imageFilter: filter })
  const handleSlideChange = (index: number) => setCurrentImageIndex(index)

  const previewImages = imagePreviews.map((src, index) => ({ src, alt: `Preview ${index + 1}` }))
  const currentFilter: FilterKey = imagesEditData[currentImageIdx]?.imageFilter ?? 'normal'
